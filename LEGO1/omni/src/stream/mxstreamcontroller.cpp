...
MxResult MxStreamController::VTable0x24(MxDSAction* p_action)
{
	...
	if (m_action0x60 == NULL) {
		...
		p_action->SetInstanceId(m_action0x60->GetInstanceId());
...
MxResult MxStreamController::FUN_100c1800(MxDSAction* p_action, MxU32 p_val)
{
	MxNextActionDataStart* dataActionStart =
		new MxNextActionDataStart(p_action->GetObjectId(), p_action->GetInstanceId(), p_val);
	...
	if (p_action->GetInstanceId() == -1) {
		...
		for (MxDSObjectList::iterator it = m_unk0x54.begin(); it != m_unk0x54.end(); it++) {
			...
			if (action->GetObjectId() == p_action->GetObjectId()) {
				newUnknown24 = Max(newUnknown24, action->GetInstanceId());
			}
		}
		...
		p_action->SetInstanceId(newUnknown24 + 1);
	}
	else {
...
MxResult MxStreamController::VTable0x30(MxDSAction* p_action)
{
	...
	if (action != NULL) {
		MxNextActionDataStart* data = m_nextActionList.FindAndErase(action->GetObjectId(), action->GetInstanceId());
		delete action;
		delete data;
		result = SUCCESS;
	}
...
MxResult MxStreamController::FUN_100c1f00(MxDSAction* p_action)
{
	...
	if (chunk->SendChunk(m_subscribers, FALSE, p_action->GetInstanceId()) != SUCCESS) {
		delete chunk;
	}
...
{
	return m_nextActionList.Find(p_action->GetObjectId(), p_action->GetInstanceId());
}
