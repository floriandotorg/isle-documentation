...
MxLong Isle::HandlePathStruct(LegoPathStructNotificationParam& p_param)
{
	...
	if (result == 0) {
		...
		case 0x12c:
			...
			break;
		case 0x12d:
			AnimationManager()->SetCharacterDurationAndUnkown10("brickstr", 0, 20000);
			result = 1;
			break;
		case 0x131:
			...
			break;
		case 0x132:
			AnimationManager()->SetCharacterDurationAndUnkown10("mama", 0, 20000);
			AnimationManager()->SetCharacterDurationAndUnkown10("papa", 0, 20000);
			result = 1;
			break;
		case 0x136:
	...
	if (p_enable) {
		...
		case Act1State::e_transitionToJetski: {
			...
			if (raceState->m_state == RaceState::e_finished) {
				...
				AnimationManager()
					->PlayAnimation(script, NULL, TRUE, LegoAnimationManager::e_true, NULL, FALSE, FALSE, TRUE, FALSE);
			}
